...
internal class ProgressTextOverlayVertical @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        if (customFontFamily!=0) setCustomFontFamily(customFontFamily)
...
//            val yPosition = (height) / 2 + (textContainerHeight / 2)
            val xPosition = (width) / 2 - (textContainerWidth / 2) + textSidePadding
...
//                yPosition = (height * progressValue) + textSidePadding
                yPosition = 0f + height - 10
...
//            val newTypeface = Typeface.createFromAsset(context.assets, customFontPath)
            val newTypeface = ResourcesCompat.getFont(context,newFontPath.toInt())
    ...
    fun setCustomFontFamily(newFontFamily: Int) {
        if (newFontFamily!=0) {
            this.customFontFamily = newFontFamily
//            val newTypeface = Typeface.createFromAsset(context.assets, customFontFamily)
            val newTypeface = ResourcesCompat.getFont(context,newFontFamily)
            progressTextOverlayPaint.typeface = newTypeface
            backgroundTextOverlayPaint.typeface = newTypeface
