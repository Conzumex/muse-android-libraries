...
public class SleepStageRangeGraph extends View {
    ...
    Paint mRangePaint,mRangeDimPaint;
    int barHeight = 75;
    int radiusBar = 7;
    float lineWidth = 4f;
    float textSize = 25;
    ...
    //for ranges
    int rangeLineColor = Color.parseColor("#ffffff");
    float rangeLineWidth = 5f;
    int rangeDimColor = Color.parseColor("#bb000000");
    int rangeStartIcon = R.drawable.ic_sleep_range_start;
    int rangeEndIcon = R.drawable.ic_sleep_range_end;
...
//    List<Float> xValues,yValues;
    ...
    float rangeStartX=-1, rangeEndX=-1;
    float lastMarkerX=-1;
    ...
    public SleepStageRangeGraph(Context context) {
        this(context, null, 0);
    }

    public SleepStageRangeGraph(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SleepStageRangeGraph(Context context, AttributeSet attrs, int defStyle) {
        ...
        initPaint();
        chartOffsetTop = chartOffsetTop + (rangeIconSize*2);
...
//        entries.add(new SleepEntry(3,1));
    ...
    void initPaint(){
        ...
        mRangePaint.setColor(rangeLineColor);
        mRangePaint.setStrokeWidth(rangeLineWidth);

        mRangeDimPaint = new Paint();
        mRangeDimPaint.setColor(rangeDimColor);
        mRangeDimPaint.setStyle(Paint.Style.FILL);
    }
...
//                tempDeb = tempDeb+" {x:"+startX+", prevY:"+prevBarLineY+" ,ypos "+yPos +"} ";
    ...
    void drawLabels(Canvas canvas){
        ...
        canvas.drawText(labelXFormatter.getLabel(xValueStart),xPosStart+offsetLabelCuttoff,chartHeight+axisSpaceX+textSize,labelPaint);
        ...
        int offsetLabelCuttoffEnd = setEdgeLabelAligned?0:labelCuttWidthEnd>chartWidth?(int)(labelCuttWidthEnd-chartWidth):0;
        canvas.drawText(labelXFormatter.getLabel(xValueEnd),xPosEnd-offsetLabelCuttoffEnd,chartHeight+axisSpaceX+textSize,labelPaint);
    ...
    void drawRanges(Canvas canvas){
        ...
        canvas.drawLine(rangeEndXDot,chartHeight,rangeEndXDot,chartRangeTopMax,mRangePaint);

        int dimTopPos = 0 + chartOffsetTop + (int)(edgeLineWidth);
        RectF rect = new RectF(0+axisSpace+axisWidth,chartHeight,rangeStartXDot-(rangeLineWidth/2),dimTopPos);
        canvas.drawRect(rect,mRangeDimPaint);
        RectF rectEnd = new RectF(rangeEndXDot+(rangeLineWidth/2),chartHeight,chartGraphEndX+(chartPaddingH/2)+axisSpace-edgeLineWidth,dimTopPos);
        canvas.drawRect(rectEnd,mRangeDimPaint);

        Drawable d = AppCompatResources.getDrawable(getContext(), rangeStartIcon);
        if (d != null){
            d.setBounds((int)rangeStartXDot-(rangeIconSize/2), chartRangeTopMax-rangeIconSize-rangeIconPadding, (int)((rangeIconSize/2)+rangeStartXDot), chartRangeTopMax-rangeIconPadding);
            d.draw(canvas);
        }
        Drawable dEnd = AppCompatResources.getDrawable(getContext(), rangeEndIcon);
        if (dEnd != null){
            dEnd.setBounds((int)rangeEndXDot-(rangeIconSize/2), chartRangeTopMax-rangeIconSize-rangeIconPadding, (int)((rangeIconSize/2)+rangeEndXDot), chartRangeTopMax-rangeIconPadding);
            dEnd.draw(canvas);
        }
    }
