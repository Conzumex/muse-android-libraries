...
import com.conzumex.clockrangepicker.utils.dpToPx
import com.conzumex.clockrangepicker.utils.pxToDp
import kotlin.math.cos
import kotlin.math.sin

@Keep
class DefaultClockRenderer(private val picker: TimeRangePicker): BitmapCachedClockRenderer {
    private val _minuteTickWidth = dpToPx(0f)
    private val _hourTickWidth = dpToPx(1f)
...
//            canvas.drawLine(startX, startY, stopX, stopY, _tickPaint)
    ...
    private fun drawLabels(canvas: Canvas) {
        ...
        for (i in labels.indices) {
            ...
            if(angle==90f)
                //to move the label in bottom of clock
                canvas.drawText(
                    label,
                    position.x,
                    position.y + bounds.height()/2.7f,
                    _labelPaint
                )
            else
                canvas.drawText(
    ...
    companion object {
        ...
        private val LABELS_SAMSUNG_24 = arrayOf("12 am", "6 am", "12 pm", "6 pm")
